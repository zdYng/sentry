...
describe('Avatar', function() {
  ...
  describe('render()', function() {
    it('should show a gravatar when avatar type is gravatar', function(done) {
      ...
      let wrapper = mount(<Avatar user={user} />);
      let avatar = wrapper.instance();
      setTimeout(() => {
        wrapper.update();
        sandbox.stub(avatar, 'buildProfileUrl');
        avatar.renderImg();
        expect(avatar.buildProfileUrl.called).not.toBeTruthy();
        expect(wrapper.state('gravatarUrl')).toBeTruthy();
        done();
      }, 1);
    });
